    ...
    try {
      ...
      if (err.code !== 'auth/popup-closed-by-user' && err.code !== 'auth/cancelled-popup-request') {
        if (err.code === 'auth/unauthorized-domain') {
          setError(
            `Domain Unauthorized: "${window.location.hostname}" must be added to Authorized Domains. Go to Firebase Console -> Authentication -> Settings -> Authorized domains and add "${window.location.hostname}".`
          );
        } else if (err.code === 'auth/internal-error') {
          setError(
            `Google Sign-In internal error: Please verify that "${window.location.hostname}" is added in Firebase Console (Authentication > Settings > Authorized Domains) and Google Cloud OAuth Client Credentials (Authorized JavaScript origins).`
          );
        } else {
